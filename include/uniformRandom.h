...
template<typename RandomGenerator = std::default_random_engine> struct uniformRandom {
    ...
    template<typename Iter> Iter operator()(Iter start, Iter end) { return select(start, end); }

    // convenience function that works on anything with a sensible begin() and end(), and returns
    // with a ref to the value type
